...
class MyDataset(Dataset):
    def __init__(self,
        ...
        self.train_dataset_with_no_paddding, \
        self.attention_window = self.pre_option(path,longformer_w_ratio)
    ...
    # 数据预处理
    def pre_option(self, path: str, longformer_w_ratio:float):
        """
        ...
        for item in test_data:
            ...
            if item.shape[1] > max_lenth:
                max_lenth = item.shape[1]
        # +++++++++++++++++++Special part in longformer data processing+++++++++++++
        attention_window = int(max_lenth*longformer_w_ratio)
        seqlen = max_lenth
        if  seqlen % (attention_window * 2) != 0:
            padding_len = (attention_window * 2 - seqlen % (attention_window * 2)) % (attention_window * 2)
            max_lenth=max_lenth+padding_len
            # if padding_len > 0:
        ...
        return train_len, test_len, input, channel, output_len, train_dataset, train_label, test_dataset, test_label, max_length_sample_inTest, train_dataset_with_no_paddding,attention_window