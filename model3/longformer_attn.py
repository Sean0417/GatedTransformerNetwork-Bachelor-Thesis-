...
class LongformerSelfAttention(nn.Module):
    def __init__(self, config, layer_id,device):
        ...
        self.query = nn.Linear(config.hidden_size, self.embed_dim).to(device)
        self.key = nn.Linear(config.hidden_size, self.embed_dim).to(device)
        self.value = nn.Linear(config.hidden_size, self.embed_dim).to(device)
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            if max_num_extra_indices_per_batch <= 0:
                ...
                # To support the case of variable number of 
                #  attention in the rows of a batch,
        ...
        outputs = (context_layer, attn_weights.squeeze(0)) if output_attentions else (context_layer,)
        return outputs